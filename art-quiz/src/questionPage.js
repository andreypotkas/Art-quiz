...
import { callTimer, tentacles } from "./settings";
import images from "./assets/image-data/images.js";
import { resultButtonCard } from "./artistsQuiz";
export let trueAns = new Audio(
  "../src/assets/jg-032316-sfx-elearning-correct-answer-sound-1.mp3"
);
export let wrongAns = new Audio("../src/assets/z_uk-proigrysha.mp3");
export let finallySound = new Audio("../src/assets/pole-chudes-priz.mp3");
let questionCounter = 0;
let trueQuestionCounter = 0;
export let timer = document.getElementById("timer-container");

function createQuestionPage(numCategory) {
  const tentacles = document.getElementById("tentacles");
  let count = tentacles.value;
  ...
  buttonContainer.onclick = function (event) {
    ...
    if (target.classList.contains("true-answer")) {
      ...
      !target.classList.contains("true-answer") &&
...
function createModal(target, answer, color, numCategory) {
  ...
  nextQuestionBtn.onclick = function (event) {
    ...
    if (target.classList.contains("next-page-btn") && questionCounter == 10) {
      ...
      artistsQuizContainer.querySelectorAll('.result-button-card')[currCategory].classList.remove('hide');
      artistsQuizContainer.querySelectorAll(".text-card")[
        currCategory
      ].textContent = `${trueQuestionCounter}`;
      artistsQuizContainer
        .querySelectorAll(".text-card")
        [currCategory].classList.remove("hide");
      artistsQuizContainer
        .querySelectorAll(".artists-quiz-card-container")
        .forEach((e) => {
...
function modalCongratulations() {
  ...
  //imgQuestionModal.classList.add('img-question');
  modalContentDescription.innerHTML = `<div>congratulations</div>
